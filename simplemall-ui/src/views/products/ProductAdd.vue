...
import { addProduct } from '@/api/ProductAdd';
...
onMounted(async () => {
  ...
  try {
    ...
    const categoryData = response.data; // 假设返回的数据在 response.data 中
...
const submitForm = async () => {
  ...
  if (product.value.pname === '' || product.value.pprice === null || product.value.pnumber === null ){
  ...
  try {
    ...
    console.error("Catch block error:", error); // 打印捕获的错误
    ElMessage.error('添加失败，请重试。');
  }
};
